 ...
 */
package com.dgtt.core.constants;

/**
 *
 * @author sadfsafbhsaid
 */
public enum StatusInterferenceInfoEnum {
    CHO_TIEP_NHAN(1L, "Chờ tiếp nhận"),CHO_PHAN_CONG(2L, "Chờ phân công");
    private final Long status;
    private final String statusDescription;

    private StatusInterferenceInfoEnum(Long status, String statusDescription) {
        this.status = status;
    ...
    public static String getStatusDescription(Long status) {
        for (StatusInterferenceInfoEnum statusDescription : StatusInterferenceInfoEnum.values()) {
